...
def update_to_512_GB_storage() -> None:
    laptops_to_update = Laptop.objects.filter("Asus", "Lenovo")
    laptops_to_update.update(storage=512)

def update_to_16_GB_memory() -> None:
    laptops_to_update = Laptop.objects.filter("Apple", "Dell", "Acer")
    laptops_to_update.update(memory=16)

def update_operation_system() -> None:
    Laptop.objects.update(
        operation_system=Case(
            When(brand="Asus", then="Windows"),
            When(brand="Apple", then="MacOS"),
            When(brand="Dell", then="Linux"),
            When(brand="Acer", then="Linux"),
...
def change_chess_games_drawn() -> None:
    ChessPlayer.objects.all().update(drawn=10)
...
#     Meal.objects.update(
#         chef=Case(
#             When(meal_type=MealTypeChoice.BREAKFAST, then=Value("Gordon Ramsаy")),
#             When(meal_type=MealTypeChoice.LUNCH, then=Value("Julia Child")),
#             When(meal_type=MealTypeChoice.DINNER, then=Value("Jamie Oliver")),
#             When(meal_type=MealTypeChoice.SNACK, then=Value("Thomas Keller"))
#         )
...
def update_dungeon_bosses_health() -> None:
    Dungeon.objects.exclude(DungeonDifficultyChoice.EASY).update(boss_health=500)

def update_dungeon_recommended_levels() -> None:
    Dungeon.objects.update(
        dungeon_recommended_level=Case(
            When(DungeonDifficultyChoice.EASY, then=Value(25)),
            When(DungeonDifficultyChoice.MEDIUM, then=Value(50)),
            When(DungeonDifficultyChoice.HARD, then=Value(75)),
        )
    )

def update_dungeon_rewards() -> None:
    Dungeon.objects.update(
        dungeon_rewards=Case(
            When(boss_health=500, then=Value("1000 Gold")),
            When(name__startswith="E", then=Value("New dungeon unlocked")),
            When(name__endswith="s", then=Value("Dragonheart Amulet"))
        )
    )
...
def show_workouts() -> str:
    workouts = Workout.objects.filter(workout_type__in=[WorkoutTypeChoice.CALISTHENICS, WorkoutTypeChoice.CROSSFIT]).order_by("id")
...
def get_high_difficulty_cardio_workouts() -> None:
    workouts = Workout.objects.filter(workout_type=WorkoutTypeChoice.CARDIO, difficulty=WorkoutDifficultyChoice.HARD).order_by("instructor")
